...
<form id="customerForm" class="Form" action="" method="Get">
 <h3>Search Customer</h3>
    ...
    <select placeholder="Active/Inactive" style="margin-right:20px;background-color:white" name="status"  id="s" onchange="fetchStatusData();showCustomerTable()">
    <option>Active</option>
    <option>Inactive</option>
    </select><br><br>
    
        
        <input type="button" value="View" class="submit" onclick="showCustomerTable()" />
       ...
       <table class="table-bordered" id="customerData" style="display:none">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:black; text-decoration:none"><a href='update?cid=<%=cmodel.getId()%>&cname=<%=cmodel.getName()%>&cemail=<%=cmodel.getEmail()%>&ccontact=<%=cmodel.getContact()%>&caddress=<%=cmodel.getAddress()%>&status=<%=cmodel.getStatus()%>'><%=cmodel.getId()%></a></td> <td><%=cmodel.getName() %></td>
